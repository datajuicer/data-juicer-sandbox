...
from data_juicer_sandbox.data_pool_manipulators import (
...
from data_juicer_sandbox.evaluators import Gpt3QualityEvaluator, InceptionEvaluator
from data_juicer_sandbox.model_executors import (
...
class DataEvaluatorFactory(object):
    ...
    def __call__(self, eval_cfg: dict = None, *args, **kwargs):
        ...
        if eval_cfg.type == "vbench_video_evaluator":
            from data_juicer_sandbox.specific_hooks.text_to_video.model_hooks import (
                VBenchEvaluator,
            )
            ...
            from data_juicer_sandbox.evaluators import AccuracyEvaluator

            evaluator = AccuracyEvaluator(eval_cfg)
        elif eval_cfg.type == "mse":
            from data_juicer_sandbox.evaluators import MSEEvaluator
...
class GeneralDataExecutorFactory(object):
    def __call__(self, data_exec_cfg: dict = None, *args, **kwargs):
        ...
        if data_exec_cfg.type == "coco_caption_to_dj_conversion":
            from data_juicer_sandbox.specific_hooks.intervl_coco_captioning.preparation_hooks import (
            ...
            from data_juicer_sandbox.specific_hooks.text_to_video.preparation_hooks import (
...
class ModelEvaluatorFactory(object):
    ...
    def __call__(self, eval_cfg: dict = None, *args, **kwargs):
        ...
        if eval_cfg.type == "internvl_coco_caption":
            from data_juicer_sandbox.specific_hooks.intervl_coco_captioning.model_hooks import (
...
class ModelTrainExecutorFactory(object):
    def __call__(self, model_cfg: dict = None, *args, **kwargs):
        ...
        if model_cfg.type == "modelscope":
            ...
            from data_juicer_sandbox.specific_hooks.rft.model_hooks import (
...
class ModelInferExecutorFactory(object):
    def __call__(self, generate_cfg: dict = None, *args, **kwargs):
        ...
        if generate_cfg.type == "easyanimate":
            ...
            from data_juicer_sandbox.model_executors import HFTransformersInferExecutor

            return HFTransformersInferExecutor(generate_cfg, **kwargs)
        elif generate_cfg.type == "vllm":
            from data_juicer_sandbox.model_executors import VLLMInferExecutor

            return VLLMInferExecutor(generate_cfg, **kwargs)
        elif generate_cfg.type == "api":
            from data_juicer_sandbox.model_executors import APIModelInferExecutor
